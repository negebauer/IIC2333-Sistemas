...
void
test_priority_high_load_yield(void) {
  /* Make sure our priority is the default. */
  ASSERT(thread_get_priority() == PRI_DEFAULT);

  msg("Starting 5 Busy threads");
  thread_create("#######", PRI_MAX-1,     &quite_busy, NULL);
  thread_create("-------", PRI_MAX-5,     &quite_busy, NULL);
  thread_create("___",     PRI_DEFAULT-1, &quite_busy, NULL);
  thread_create("+++",     PRI_DEFAULT+1, &quite_busy, NULL);
  thread_create("---",     PRI_DEFAULT,   &quite_busy, NULL);
